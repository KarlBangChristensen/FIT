...
%macro AMTS_simu(samplesize, from, to, seed=0);
	...
	ods exclude all;
	proc sql noprint;
		select mean(WLE), std(WLE) into :latentmean, :latentSD
		from FIT.pp_amts_cml_outdata;
	quit;
	%let latentmean=&latentmean.;
	%let latentSD=&latentSD.;

	%do sim=&from %to &to;
		%do id=1 %to &samplesize.;
			data d&id;
				WLE=&latentmean.+&latentSD.*ranuni(0);
			run;;			
		%end;
