...
This example uses the responses of 197 persons to the ten questions of the Abbreviated Mental Test Score \cite[AMTS]{hodkinson72}. The AMTS is used in screening for dementia. One point is given for each correct answer; a score of 6 or less suggests that the patient has some mental impairment. 
...
- The use of a single critical value for misfit diagnosis for item fit statistics based on residuals will, across different conditions, lead to both the
overdetection and underdetection of misfit.

- Wolfe (2013): explains how three implementations of the bootstrap procedure works

- Dimitrov, Smith - person fit. In the context of person fit Magis et al adapted a general approach proposed by Snijders (2001) to correct infit and outfit indexes and compared corrected indexes are compared to their classical versions using a simulation study. The correction was found to control Type I errors, while significantly increasing power to detect specific misfitting response patterns gets 

Karabatsos \cite{} provided a critical analysis of these illustrating that the task of Rasch fit analysis is not as simple and straightforward as it appears to be. The faulty statistical properties of the residual fit statistics do not allow either a convenient or a straightforward approach to Rasch fit analysis. For any of these fit statistic, the use of a single minimum critical value for misfit diagnosis across different testing situations, where the situations vary in sample and test properties, leads to both the overdetection and underdetection of misfit. 

- Karabatsos advocated the use of residual-free Rasch fit statistics based on the number of Guttman response errors and for the dichotomous rasch model Kreiner and Christensen studied exact
tests based on

- The empirical standard deviations of the infit and outfit mean squares become smaller as the sample size becomes larger

- Simulation studies have shown means of t-transformed infit and outfit item fit statistics to be very close to zero, but to have standard deviations considerably smaller than one. Moreover, standard deviations of these became smaller as the number of items increased. 
